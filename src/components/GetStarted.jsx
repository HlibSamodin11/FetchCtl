...
export default function GetStarted({ onClose }) {
  ...
  async function handleAuth(e) {
    ...
    try {
      ...
      setLoading(false);
      setTimeout(() => { cooldownRef.current = false; }, authMode === 'login' ? 1500 : 3000);
    }
  }
        ...
        {success ? (
          ...
          <>
            ...
            <div className="flex flex-col gap-2 mb-4">
              ...
              >
                <svg className="w-4 h-4"><use href="/sprite.svg#icon-google" /></svg>
              ...
              <button
                ...
                className="w-full py-2.5 flex items-center justify-center gap-3 bg-button-bg border border-button-stroke rounded-xl text-sm font-bold text-accent-text hover:border-accent-text transition-all cursor-pointer"
              >
                <svg className="w-4 h-4 fill-accent-text"><use href="/sprite.svg#icon-github" /></svg>
            ...
            <p className="mt-5 text-center text-xs text-main-text">
              {authMode === 'login' ? (
                <>Don't have an account?{' '}
                  <button onClick={() => { setAuthMode('register'); setAuthError(''); }} className="text-accent-text underline cursor-pointer">Register</button>
                </>
              ) : (
                <>Already have an account?{' '}
                  <button onClick={() => { setAuthMode('login'); setAuthError(''); }} className="text-accent-text underline cursor-pointer">Log in</button>
                </>
              )}
            </p>
          </>
