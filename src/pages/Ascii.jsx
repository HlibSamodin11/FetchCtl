      ...
      >
        {copied ? (
          <svg className="w-4 h-4">
            <use href="/sprite.svg#icon-tick" className="fill-accent-text" />
          </svg>
        ) : (
          <svg className="w-5 h-5">
            <use href="/sprite.svg#icon-copy" className="fill-accent-text" />
          </svg>
        )}
      </button>
        ...
        <div className="flex gap-5 py-1 flex-wrap">
          <p className="flex items-center gap-2 text-reverse/50">
            <svg className="h-3 w-3">
              <use href="/sprite.svg#icon-user" className="fill-reverse/50" />
            </svg>
            FetchCtl (web)
          </p>
          <button
          ...
          >
            <svg className="h-3 w-3">
              <use
                ...
                className={liked ? 'fill-[#E85555]' : 'fill-reverse/30'}
              />
            </svg>
            <span className={liked ? 'text-[#E85555]' : 'text-reverse/50'}>
              {likes}
            </span>
          </button>
          <p className="flex items-center gap-2 text-reverse/50">
            <svg className="h-4 w-4">
              <use href="/sprite.svg#icon-eye" className="fill-reverse/50" />
            </svg>
            {views ?? '...'}
          </p>
          <p className="flex items-center gap-2 text-reverse/50">
            <svg className="h-3 w-3">
              <use href="/sprite.svg#icon-clock" className="fill-reverse/50" />
            </svg>
            3 days ago
          </p>
        </div>
    ...
    <section className="bg-bg py-20 flex flex-col items-center px-6 md:px-12">
      <AsciiFilters items={asciiFile.ascii}>
        {(filtered) => {
          if (!filtered.length)
            return (
              <p className="text-main-text text-sm text-center">
                no results found
              </p>
            );
