...
export default function Profile({ user }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      setFollowersList(
        (followersRaw || []).map((r) => r.profiles).filter(Boolean),
      );
      ...
      setFollowingList(
        (followingRaw || []).map((r) => r.profiles).filter(Boolean),
      );
              ...
              {isOwn && (
                <>
                  <button
                    ...
                    className="text-xs px-4  rounded-xl bg-get-started-bg text-get-started-text font-bold hover:opacity-80 transition font-jetbrains"
                  >
                    + post
                  </button>
                </>
          ...
          <div className="flex flex-col lg:flex-row gap-5 items-start">
            <div className="flex-1 min-w-0 flex flex-col gap-5 w-full">
              ...
              <div className="bg-area border border-area-border rounded-2xl p-4 sm:p-5">
                <h2 className="text-accent-text font-bold text-base mb-4">
                  Pinned Post
                </h2>
                {pinnedPost ? (
                  <Link
                    to={`/post/${pinnedPost.id}`}
                  ...
                  >
                    ...
                    <div className="flex flex-col justify-center gap-2">
                      <p className="text-accent-text font-medium group-hover:underline">
                        {pinnedPost.title}
                      </p>
                      {pinnedPost.description && (
                        <p className="text-main-text text-sm line-clamp-2">
                          {pinnedPost.description}
                        </p>
                      )}
                      <p className="text-main-text text-xs">
                        {timeAgo(pinnedPost.created_at)}
                      </p>
                    </div>
                  </Link>
                ) : (
              ...
              {/* recently liked */}
              <div>
                <h2 className="text-accent-text font-bold text-base mb-3">
                  Recently Liked
                </h2>
                <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-none">
                  {recentLiked.length > 0
                    ...
                    : [0, 1, 2, 3].map((i) => (
                        <div
                          key={i}
                          className="w-[110px] sm:w-[120px] h-[85px] sm:h-[90px] bg-accent-bg border border-area-border rounded-xl flex-shrink-0"
                        />
                      ))}
                </div>
              </div>
                ...
                <div className="flex gap-4">
                  {recentPosts.length > 0
                    ...
                    : [0, 1].map((i) => (
                        <div
                          key={i}
                          className="flex-1 h-[140px] sm:h-[160px] bg-accent-bg border border-area-border rounded-xl"
                        />
                      ))}
                </div>
              </div>
            ...
            <div className="w-full lg:w-[220px] flex-shrink-0 flex flex-row lg:flex-col gap-3 flex-wrap">
              <div className="bg-area border border-area-border rounded-2xl p-4 sm:p-5 flex-1 lg:flex-none">
                <div className="flex flex-col gap-1 mb-3">
                  <button
                    ...
                    className="text-left text-accent-text text-lg hover:underline"
                  >
                    <span className="font-bold">{followerCount}</span>
                    <span className="text-base font-normal text-main-text">
                      {" "}
                      Followers
                    </span>
                  </button>
              ...
              <div className="bg-area border border-area-border rounded-2xl p-4 sm:p-5 flex-1 lg:flex-none">
                <h3 className="text-accent-text font-bold text-base mb-3">
                  Achievements
                </h3>
              ...
              {profile.website && (
                <div className="bg-area border border-area-border rounded-2xl p-4 sm:p-5 flex-1 lg:flex-none">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-accent-text text-sm hover:underline break-all"
                  >
                    {profile.website.replace(/^https?:\/\//, "")}
                  </a>
                </div>
        ...
        {activeTab === "Posts" && (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
            {posts.length > 0 ? (
              ...
              <p className="text-main-text text-sm col-span-2 md:col-span-3">
                no posts yet.
              </p>
            )}
          </div>
