      ...
      <div className="px-6 md:px-12">
        <div className="container mx-auto">
          ...
          <div className="mb-6 flex flex-col gap-1">
            ...
            {profile.bio && (
              <p className="text-main-text/70 text-sm mt-2 max-w-md leading-relaxed">{profile.bio}</p>
            )}
            ...
            {likedArts.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 gap-3">
                ...
                <p className="text-main-text/30 text-sm">
                  {isOwn ? "you haven't liked anything yet" : 'no liked art yet'}
                </p>
                {isOwn && (
                  <button
                    onClick={() => navigate('/ascii')}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {likedArts.map(item => (
                  <AsciiCard key={item.id} item={item} user={currentUser} onOpenLogin={() => {}} />
                ))}
              </div>
            )}
          </div>
